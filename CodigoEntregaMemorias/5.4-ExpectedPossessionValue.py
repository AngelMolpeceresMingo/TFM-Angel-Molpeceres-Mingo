...
def calculate_sog_sgg_events(current_coords, prev_coords, space_value_map, fps, all_excluded_ids):
    ...
    for p_curr in current_coords_filtered:
        ...
        x_px_curr, y_px_curr = min(w-1, max(0, int(p_curr['x']/105*w))), min(h-1, max(0, int(p_curr['y']/68*h)))
        x_px_prev, y_px_prev = min(w-1, max(0, int(p_prev['x']/105*w))), min(h-1, max(0, int(p_prev['y']/68*h)))
...
def calculate_epv_simplified(coords, ball_carrier_id, all_excluded_ids, attacking_team_color=None, attack_direction='right'):
    ...
    field_length = 105.0
    ball_x = max(0, min(105, ball_carrier['x']))
    ball_y = max(0, min(68, ball_carrier['y']))
    ...
    if attacking_team_color is not None:
        attackers = [p for p in coords if p['color'] == attacking_team_color and 0 <= p['x'] <= 105 and 0 <= p['y'] <= 68]
        defenders = [p for p in coords if p['color'] != attacking_team_color and p['player_id'] not in all_excluded_ids and 0 <= p['x'] <= 105 and 0 <= p['y'] <= 68]
    else:
        is_team1_attacking = 1 <= ball_carrier_id <= 11
        attackers = [p for p in coords if (1 <= p['player_id'] <= 11 if is_team1_attacking else 12 <= p['player_id'] <= 22) and 0 <= p['x'] <= 105 and 0 <= p['y'] <= 68]
        defenders = [p for p in coords if (12 <= p['player_id'] <= 22 if is_team1_attacking else 1 <= p['player_id'] <= 11) and 0 <= p['x'] <= 105 and 0 <= p['y'] <= 68]
...
def visualize_vertical_view(image_folder, detections_path, real_coords_path, fps=10, output_width=1280, exclude_ids=None, ball_ids=None, attacking_team_sample_ids=None):
    ...
    while frame_idx < len(image_files):
        ...
        if len(epv_history) == HISTORY_LENGTH:
            ...
            if epv_change > EPV_INCREASE_THRESHOLD:
                ...
                for p_curr in current_coords:
                    p_prev = players_by_id_prev.get(p_curr['player_id'])
                    if (p_prev and p_curr['color'] == attacking_team_color and 
                        0 <= p_curr['x'] <= 105 and 0 <= p_curr['y'] <= 68 and
                        0 <= p_prev['x'] <= 105 and 0 <= p_prev['y'] <= 68):
                        
                        dx = p_curr['x'] - p_prev['x']
                        if dx > max_dx:
                            max_dx, best_advancer_id = dx, p_curr['player_id']
                        
                        if p_curr['x'] > max_pos_x:
                            max_pos_x, best_leader_id = p_curr['x'], p_curr['player_id']
                
                players_to_trail = set([best_advancer_id, best_leader_id])
                for player_id in players_to_trail:
                    if player_id != -1:
                        if player_id not in value_adding_runs:
                            value_adding_runs[player_id] = {'trail': deque(maxlen=VALUE_RUN_TRAIL_LENGTH), 'timer': 0}
                        value_adding_runs[player_id]['timer'] = VALUE_RUN_TRAIL_LENGTH

        for player_id, data in list(value_adding_runs.items()):
            player_pos = players_by_id_curr.get(player_id)
            if player_pos and 0 <= player_pos['x'] <= 105 and 0 <= player_pos['y'] <= 68:
                x_px = max(0, min(field_width_px - 1, int(player_pos['x']/field_dims_m[0]*field_width_px)))
                y_px = max(0, min(field_height_px - 1, int(player_pos['y']/field_dims_m[1]*field_height_px)))
        ...
        status_text = f"Frame: {frame_id} ({frame_idx+1}/{len(image_files)}) | FPS: {fps} | SOG/SGG: {'ON' if SOG_SGG_ENABLED else 'OFF'} | EPV: {'ON' if EPV_ENABLED else 'OFF'}{' [PAUSADO]' if paused else ''}"
        ...
        cv2.putText(field_2d_view, "Vista 2D: Ocupacion y Generacion de Espacio", (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        ...
        if key == ord('q'):
            ...
            SOG_SGG_ENABLED = not SOG_SGG_ENABLED
        elif key == ord('e'):
            EPV_ENABLED = not EPV_ENABLED
...
if __name__ == '__main__':
    ...
    ids_a_excluir = [1,3,21,26,25]
    ids_balon = [2]
    ids_equipo_atacante_muestra = [13, 14, 15]
